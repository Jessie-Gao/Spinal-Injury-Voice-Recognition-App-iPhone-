...
- (void) pocketsphinxDidReceiveHypothesis:(NSString *)hypothesis recognitionScore:(NSString *)recognitionScore utteranceID:(NSString *)utteranceID {
    ...
    NSLog(@"\n =============Heard: %@ ============",hypothesis);
    NSArray *list = [hypothesis componentsSeparatedByString:@" "];  //判断是否有多个，返回最后一个
    if (list.count > 0) {
        if (_delegate ) {
            [_delegate received:list.lastObject];
        }
    }
//    else {
//        if (_delegate ) {
//            [_delegate received:hypothesis];
//        }
//    }
